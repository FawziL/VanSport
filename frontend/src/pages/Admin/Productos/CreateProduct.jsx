...
export default function CreateProduct() {
  ...
  const processExtraImages = (files) => {
    ...
    if (imageFiles.length > 0) {
      setExtras((prev) => [...prev, ...imageFiles]);
      const previewPromises = imageFiles.map((file) => {
        return new Promise((resolve) => {
          const reader = new FileReader();
          reader.onloadend = () => resolve({ url: reader.result, name: file.name, size: file.size });
          reader.readAsDataURL(file);
        });
      });
  ...
  const handleMainDragLeave = (e) => {
    e.preventDefault();
    if (!e.currentTarget.contains(e.relatedTarget)) setIsDragOverMain(false);
  };

  const handleExtrasClick = () => extrasInputRef.current?.click();
  ...
  const handleExtrasDragLeave = (e) => {
    e.preventDefault();
    if (!e.currentTarget.contains(e.relatedTarget)) setIsDragOverExtras(false);
  };
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">{String(error)}</div>
      )}
        ...
        <div>
          ...
          {!imagenPreview ? (
            <div className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${isDragOverMain ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'}`}
              onClick={handleMainImageClick} onDrop={handleMainDrop} onDragOver={handleMainDragOver} onDragLeave={handleMainDragLeave}>
                ...
                {extrasPreviews.map((preview, index) => (
                  <div key={index} className="relative group border border-gray-200 rounded-lg p-3 bg-white hover:shadow-md transition-shadow">
                    <button type="button" onClick={() => removeExtraImage(index)}
                      className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-700 transition-colors opacity-0 group-hover:opacity-100">×</button>
                    <img src={preview.url} alt={`Preview adicional ${index + 1}`} className="w-full h-24 object-cover rounded mb-2" />
