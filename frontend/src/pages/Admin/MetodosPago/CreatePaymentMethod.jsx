  ...
  return (
    <div style={{ maxWidth: 840, margin: '2.5rem auto', padding: '0 1rem' }}>
      ...
      <div style={{ display: 'grid', gap: 12, background: '#fff', border: '1px solid #eee', padding: 16, borderRadius: 10 }}>
        ...
        <div><label>Tipo</label><input value={form.tipo} onChange={(e) => onChange('tipo', e.target.value)} className="input" placeholder="paypal | pago_movil | efectivo | ..." /></div>
        ...
        <div><label>Config (JSON)</label><textarea value={form.config} onChange={(e) => onChange('config', e.target.value)} rows={6} className="input" placeholder='{"banco":"BANESCO","telefono":"+58-..."}' /></div>
